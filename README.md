...
GitHub Actions CI (django-app/** changes)
  ├── Run tests
  ├── Build Docker image (multi-stage, tagged with github.sha)
  ├── Push to ECR
...
| `iam` | ECS task execution role, ECS task role, Secrets Manager policy, GHA OIDC role |
...
```
test → build → push to ECR → update task definition → ecs update-service
```
...
- Images tagged with `github.sha` for full commit-level traceability — never `latest`
- New ECS task definition revision registered on every deploy with the exact image just built
- Image URI passed as job output between jobs — not read from cached repository variable
...
- Terraform outputs stored as GitHub repository variables after apply — environment agnostic
...
3. Add GitHub repository secret:
   ...
   GH_PAT            — Personal access token with variables read/write scope

4. Trigger ci-cd.yml manually via workflow_dispatch (deploy: false)
   Builds and pushes image to ECR without deploying

5. Trigger infra.yml manually via workflow_dispatch
   Provide the ECR image URI from step 4 as input
   Approve the apply in GitHub Environments → production
   Infrastructure is provisioned, outputs stored as repository variables

6. From this point forward all deployments are automated on push
```
